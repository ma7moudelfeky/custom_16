...
def _load_account_details_post_init_hook(cr, registry):
    ...
    for record in env['account.financial.report'].search(
        ...
        if record.get_metadata()[0].get(
                'xmlid') == 'om_account_accountant.account_financial_report_other_income0':
            for rec in env['account.account'].search(
                    [('account_type', '=', 'income_other')]):
                record.write({"account_ids": [(4, rec.id)]})
        elif record.get_metadata()[0].get(
                'xmlid') == 'om_account_accountant.financial_report_cost_of_revenue':
            for rec in env['account.account'].search(
                    [('account_type', '=', 'expense_direct_cost')]):
                record.write({"account_ids": [(4, rec.id)]})
        elif record.get_metadata()[0].get(
                'xmlid') == 'om_account_accountant.account_financial_report_operating_income0':
            for rec in env['account.account'].search(
                    [('account_type', 'in', ('equity_unaffected', 'income'))]):
                record.write({"account_ids": [(4, rec.id)]})
        elif record.get_metadata()[0].get(
                'xmlid') == 'om_account_accountant.account_financial_report_expense0':
            for rec in env['account.account'].search(
                    [('account_type', 'in',
                      ('expense', 'expense_depreciation'))]):
                record.write({"account_ids": [(4, rec.id)]})
        elif record.get_metadata()[0].get(
                'xmlid') == 'om_account_accountant.account_financial_report_assets0':
            for rec in env['account.account'].search(
                    [('account_type', 'in', (
                ...
                'xmlid') == 'om_account_accountant.account_financial_report_liability0':
            for rec in env['account.account'].search(
                    [('account_type', 'in', (
