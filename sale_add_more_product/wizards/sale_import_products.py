    ...
    def create_items(self):
        for wizard in self:
            for product in wizard.products:
                self.env["sales.import.products.items"].create(
                    {"wizard_id": wizard.id, "product_id": product.id}
                )
        view = self.env.ref("sale_add_more_product.view_import_product_to_sale2")
        return {
        ...
        for wizard in self:
            ...
            if sale:
                for item in wizard.items:
                    ...
                    if vals:
                        sal_line = self.env["sale.order.line"].create(vals)
                        sal_line._compute_price_unit()
                        sal_line._onchange_product_id_warning()
                        sal_line._compute_discount()
                        sal_line._compute_amount()

        return {"type": "ir.actions.act_window_close"}


class SaleImportProductsItem(models.TransientModel):
    _name = "sales.import.products.items"
    _description = "Sale Import Products Items"

    wizard_id = fields.Many2one(string="Wizard", comodel_name="sales.import.products")
    product_id = fields.Many2one(
        string="Product", comodel_name="product.product", required=True
    )
    quantity = fields.Float(
        digits="Product Unit of Measure", default=12.0, required=True
    )
